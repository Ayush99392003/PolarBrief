...
def clean_text(text: str) -> str:
    if not isinstance(text, str):
        text = str(text)
    # Normalize & strip unsupported chars for Helvetica
...
def output_files(final_output: List[Dict]) -> io.BytesIO:
    ...
    # Minimal index
    minimal = [
        {"page": i.get("page"), "citation_start": i.get("citation_start"),
         "citation_end": i.get("citation_end"), "heading": i.get("heading", "")}
        for i in final_output
    ]
    save_json("Index.json", minimal)
    # PDF Export helper
    def pdfexport(json_file, fields, pdf_file):
        ...
        pdf.output(pdf_file)
    # PDFs
    pdfexport("Index.json", ["page", "citation_start", "citation_end", "heading"], "Index.pdf")
    pdfexport("all_arguments.json",
              ["page", "citation_start", "citation_end", "heading", "summary", "polarity", "source", "timestamp"],
              "all_arguments.pdf")
    pdfexport("top_5_neutral.json",
              ["page", "citation_start", "citation_end", "heading", "summary", "polarity", "source", "timestamp"],
              "top_5_neutral.pdf")
    # Balanced PDF
    def export_balanced_pdf():
        ...
        for arg in top_pro:
            ...
            pdf.ln(2)
        pdf.set_font("Helvetica", "B", 16)
        pdf.cell(0, 10, "CON ARGUMENTS (Top 5)", align="C",ln=1)
        pdf.set_font("Helvetica", "", 12)
        for arg in top_con:
            pdf.multi_cell(0, 8, clean_text(
                f"Page: {arg['page']}\nScore: {arg['final_score']}\n"
    ...
    with zipfile.ZipFile(zip_buffer, "w") as zf:
        for f in ["all_arguments.json", "top_5_neutral.json", "balanced_arguments.json",
