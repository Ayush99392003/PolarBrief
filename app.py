    ...
    if st.button("Analyze Document"):
        ...
        with st.spinner("🔍 Processing document..."):
            try:
                ...
                st.markdown(
                    '<div class="section-header">Balanced Arguments (Top 5 PRO & Top 5 CON)</div>',
                    unsafe_allow_html=True,
                )
                for i, arg in enumerate(balanced_args, 1):
                    with st.expander(
                        ...
                        st.markdown(f"**Citation_start:** {arg['citation_start']}")
                        st.markdown(f"**Citation_end:** {arg['citation_end']}")
                        st.markdown("**Summary:**")
